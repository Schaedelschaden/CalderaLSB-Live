...
function onTrigger(player)
    -- Teleport target
--  if (player:getHP() == player:getMaxHP() and player:hasKeyItem(xi.ki.VAHZL_GATE_CRYSTAL)) then
    ...
    --      player:PrintToPlayer(string.format("You must have the Vahzl Gate Crystal and be at full HP to use that command!"),xi.msg.channel.SYSTEM_3)
            player:PrintToPlayer(string.format("You must have the Vahzl Gate Crystal!"),xi.msg.channel.SYSTEM_3)
        end
    end
end
